...
document.addEventListener('DOMContentLoaded', () => {
  ...
  let prevMousePos: Vector | null = null;
  ...
  setInterval(() => {
    ...
    if (countMoveKeysPressed() > 0) {
      ...
      if (isPressed['w']) { direction.x += 1; }
      if (isPressed['a']) { direction.y -= 1; }
      if (isPressed['s']) { direction.x -= 1; }
      if (isPressed['d']) { direction.y += 1; }
    ...
    walls.forEach((wall) => {
      ...
      switch (wallVisibility) {
        case SegmentVisibilility.Invisible: color = 'black'; break;
        case SegmentVisibilility.Visible: color = 'yellow'; break;
...
function resetSettings(ctx: CanvasRenderingContext2D) {
  ctx.fillStyle = '#f0f0f0';
  ctx.strokeStyle = 'black';
  ctx.lineWidth = 2;
}

function createRenderers(ctx: CanvasRenderingContext2D) {
  return {
...
function countMoveKeysPressed(): number {
