...
export var BlackjackPlayerAction;
(function (BlackjackPlayerAction) {
    BlackjackPlayerAction[BlackjackPlayerAction["Hit"] = 0] = "Hit";
    BlackjackPlayerAction[BlackjackPlayerAction["Stand"] = 1] = "Stand";
})(BlackjackPlayerAction || (BlackjackPlayerAction = {}));
export var BlackjackGameActionType;
(function (BlackjackGameActionType) {
    BlackjackGameActionType[BlackjackGameActionType["AddPlayerCard"] = 0] = "AddPlayerCard";
    BlackjackGameActionType[BlackjackGameActionType["AddDealerCard"] = 1] = "AddDealerCard";
    BlackjackGameActionType[BlackjackGameActionType["EndGame"] = 2] = "EndGame";
    BlackjackGameActionType[BlackjackGameActionType["Initialize"] = 3] = "Initialize";
})(BlackjackGameActionType || (BlackjackGameActionType = {}));
export var BlackjackWinner;
(function (BlackjackWinner) {
    BlackjackWinner[BlackjackWinner["Player"] = 0] = "Player";
    BlackjackWinner[BlackjackWinner["Dealer"] = 1] = "Dealer";
    BlackjackWinner[BlackjackWinner["None"] = 2] = "None";
})(BlackjackWinner || (BlackjackWinner = {}));
...
export default class BlackjackGame {
    ...
    *play() {
        if (this.inPlay) {
            throw Error('Game already in play');
        }
        this.inPlay = true;
        ...
        do {
            ...
            if ((yield availableActions) === BlackjackPlayerAction.Stand) {
                break;
            }
    ...
    removeSubscriber(subscriber) { this.subscribers = this.subscribers.filter(s => s !== subscriber); }
    addToTimeline(action) {
        this.actionTimeline.push(action);
        this.subscribers.forEach(subscriber => subscriber(action));
    }
    get timeline() { return this.actionTimeline; }
...
tests: {
    ...
    for (let state = playerInterface.next(), action;; state = playerInterface.next(action)) {
        ...
        if (!state.done) {
            console.log(`player will ${BlackjackPlayerAction[action]}`);
            continue;
        }
        break;
    }
