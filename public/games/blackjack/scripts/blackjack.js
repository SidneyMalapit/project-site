import BlackjackGame, { BlackjackGameActionType, BlackjackWinner, BlackjackWinnerDescriptions } from './BlackjackGame.js';
import { CardSuit, CardRank } from './Card.js';
...
while (true) {
    await new Promise((resolve) => {
        deal.addEventListener('click', async () => {
            ...
            for (let result = game.next(); !result.done; result = game.next(await promptPlayerAction(result.value)))
                ;
            resolve();
        }, { once: true });
    });
...
async function promptPlayerAction(available) {
    let action = null;
    ...
    while (action === null || !available.includes(action)) {
        action = await new Promise((resolve) => {
            playActionBar.addEventListener('click', ({ target }) => {
                ...
                if (!closest.dataset.actionType) {
                    throw Error('Invalid action type');
                }
                resolve(parseInt(closest.dataset.actionType));
            }, { once: true });
        });
    }
    return action;
