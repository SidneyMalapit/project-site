...
document.addEventListener('DOMContentLoaded', () => {
  const table = document.getElementById('minesweeper-field')! as HTMLTableElement;
  const game = new Game(24, 24);
  createGameTable(game, table);

  for (let i = 1; i <= 8; i++) { table.style.setProperty(`--color-${i}`, colors[i - 1]); }

  // dig event
  table.addEventListener('click', ({ target }) => {
    if (!(target instanceof HTMLElement)) { return; }
    const cellElement = target.closest('td');
    if (!(cellElement instanceof HTMLTableCellElement)) { return; }
    const coord = [cellElement.getAttribute('data-row'), cellElement.getAttribute('data-column')];
    if (coord[0] === null || coord[1] === null) { throw TypeError('row or column attribute not found'); }
    const { isUninitialized } = game;
    dig(game, (coord.map(n => parseInt(n!))) as [number, number], [], game.size / 10);
  ...
  function handleRightClick(event: MouseEvent) {
    ...
    if (!(cellElement instanceof HTMLTableCellElement)) { return; }
    const coord = [cellElement.getAttribute('data-row'), cellElement.getAttribute('data-column')];
    if (!(coord[0] ?? coord[1] ?? undefined)) { throw TypeError('row or column attribute not found'); }
    toggleFlag(game, coord.map(n => parseInt(n!)) as [number, number]);
  }
  ...
  function handleMiddleClick(event: MouseEvent) {
    ...
    digNeighborsIfFlagged(game, coord.map(n => parseInt(n!)) as [number, number], coords);
...
function displayGridString({ grid, columns }: Game) {
  const str = grid.map(cellToCharacter).join('').replace(new RegExp(`(.{${columns}})`, 'g'), '$1\n');
  return str;
}

function cellToCharacter({ count, isMine, isDug }: Cell) {
  return (isDug ? isMine ? 'X' : count || ' ' : '\u2588').toString();
}

function dig(game: Game, ...args: Parameters<typeof Game.prototype.dig>) {
  const [, modifiedCoords] = args;
  const result = game.dig(...args);
  if (!modifiedCoords) { return; }
...
function digNeighborsIfFlagged(game: Game, ...args: Parameters<typeof Game.prototype.digNeighborsIfFlagged>) {
  const [, modifiedCoords] = args;
...
function toggleFlag(game: Game, [r, c]: readonly [number, number]) {
  if (!game.toggleFlag(r, c)) { return; }
  updateCellState(getCell(r, c), game.at(r, c));
}

function createGameTable({ rows, columns, grid }: Game, { tBodies: [tbody] }: HTMLTableElement) {
...
function updateCellState(cellElement: HTMLTableCellElement, { isDug, isMine, isFlagged, count }: Cell, isUninitialized = false) {
...
function getCell(row: number, column: number) {
  ...
  if (result) { return result as HTMLTableCellElement; }
