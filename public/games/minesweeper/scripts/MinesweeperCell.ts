export default class MinesweeperCell {
   ...
   */
  dig(): boolean {
    if (this.isDug || this.isFlagged) { return false; }
    this._isDug = true;
    return true;
  }
  increment() {
    if (this.isMine) { throw TypeError('attempted to increment a mine'); }
    if (this.state === 8) { throw RangeError('adjacent mine count cannot exceed 8'); }
    this.state++;
  }
  rig() {
    if (this.isMine) { throw TypeError('attempted to rig an existing mine'); }
    this.state = -1;
  }
}
